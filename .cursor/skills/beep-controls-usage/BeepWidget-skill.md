...
- **Painter Array Bounds**: Painters must safely handle length mismatches between `Values`, `Labels`, and `Colors` arrays (e.g., repeating colors via `colors[i % colors.Count]`) to prevent data truncation
