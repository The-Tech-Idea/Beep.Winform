...
- [x] `DOCMDI-NEXT-017` `DocumentSetupWizardDialog` themed through `BeepThemesManager`: extracted `WizardPalette` adapter that maps `IBeepTheme` tokens onto wizard chrome + tile + preview surfaces, removed all hard-coded colours, added per-control `AccessibleName` / `AccessibleDescription` / `AccessibleRole` for screen-reader support, and falls back to SystemColors only under OS High Contrast or when theme discovery fails (Track A — Design-Time Polish). See `DocumentHost-MDI-Phase-08-DesignTimeWizardPolish.md`.
- [x] `DOCMDI-NEXT-018` Runtime acceptance sample shipped in `WinFormsApp.UI.Test`: new `DocumentHostMdiDemoForm` + `--demo document-host-mdi` route round-trips the design-time `DocumentSetupWizardDialog` through `BeepDocumentManager` view-swap pipeline, exercises Tabbed / Browser / Native MDI modes, validates `BeepThemesManager` ↔ `WizardPalette` integration on a real form, and gives QA a reproducible smoke-test target. Also closes Phase 08 polish item `DOCMDI-P8-032` by routing tile-selection `AccessibilityNotifyClients(SelectionAdd/StateChange)` + status-label `NameChange` announcements through a small `AnnouncingLabel` shim (Track A — Design-Time Polish). See `DocumentHost-MDI-Phase-09-RuntimeWizardDemo.md`.
- [x] `DOCMDI-NEXT-019` `BeepDocumentManager` IDisplayContainer parity polish: new `BeepDocumentManager.Presets.cs` partial ships `SetTabStylePreset(DocumentTabStyle)`, `ApplyBrowserPreset()`, `ApplyIdePreset()`, and a `CurrentTabStyle` hint property that bundle `TabStyle` + `TabPosition` + `ShowAddButton` + `CloseButtonMode` in one coherent call — the same one-call ergonomic `BeepDisplayContainer.SetTabStylePreset(TabStyle)` has had for years. Phase 09 `DocumentHostMdiDemoForm` now calls `ApplyBrowserPreset()` / `ApplyIdePreset()` directly and the reflection-based `TrySet` helper has been removed. Confirms zero IDisplayContainer surface gaps between `BeepDisplayContainer` and `BeepDocumentManager`; migration is a type swap + preset-enum rename. Includes a full migration matrix in the phase doc. See `DocumentHost-MDI-Phase-10-DisplayContainerParity.md`.
- [x] `DOCMDI-NEXT-020` Design-time tab selection landed (closes the original Phase 02 gap "I cannot select any document on design time"). `BeepDocumentHostDesigner` now subscribes `host.ActiveDocumentChanged` and promotes the active `BeepDocumentPanel` into `ISelectionService` so a tab click swaps the property grid + visual selection rectangle + toolbox drop target. A new `BeepDocumentHostDesigner.TabSelection.cs` partial overrides `GetHitTest(Point)` to claim pass-through for clicks landing on a tab strip (tabs, add/scroll/overflow buttons, group headers) so the strip's runtime mouse pipeline runs at design time and raises `TabSelected`. Defensive fallbacks `ActivateTabAt(screenPoint)` + `SelectDocumentAt(screenPoint)` are exposed as the new verb "Select Tab Under Cursor" on both the context-menu and smart-tag action list (under "Documents"). A one-shot `SyncInitialActiveDocumentSelection()` runs after `SiteAllDesignPanels()` so freshly-loaded forms land on the visible document instead of the host shell. See `DocumentHost-MDI-Phase-11-DesignTimeTabSelection.md`.
