...
{
    /// <summary>
    /// Minimal clean style accordion painter
    /// </summary>
    public class MinimalAccordionPainter : AccordionPainterBase
    {
        ...
        public override void PaintItem(Graphics g, Rectangle bounds, SimpleItem item, AccordionItemState state, AccordionRenderOptions options)
        {
            ...
            // Minimal style - only paint background on hover/select
            ...
            int currentX = bounds.Left + options.HighlightWidth + options.Padding;
            if (!string.IsNullOrEmpty(item?.ImagePath) && File.Exists(item.ImagePath))
            {
                ...
                Rectangle iconRect = AccordionIconHelpers.CalculateItemIconBounds(
                    ...
                    options.ItemHeight,
                    0.6f,
                    options.HighlightWidth + options.Padding);
