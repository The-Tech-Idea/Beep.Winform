...
{
    public class NewsIconBadgePainter : BaseButtonPainter
    {
        public override void Paint(AdvancedButtonPaintContext context)
        {
            ...
            // Icon circle on left
            int circleSize = bounds.Height + 4;
            int circleOverlap = 6;
            ...
            {
                using (Font iconFont = GetDerivedTextFont(context, styleOverride: System.Drawing.FontStyle.Bold, sizeDelta: -2f))
                using (SolidBrush iconBrush = new SolidBrush(Color.White))
                using (StringFormat sf = new StringFormat { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center })
                {
                    g.DrawString(iconText, iconFont, iconBrush, circleBounds, sf);
                }
            }
        ...
        {
            if (string.IsNullOrEmpty(text)) return new[] { "LIVE", "WORLD NEWS" };
