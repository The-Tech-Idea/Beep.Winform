...
{
    /// <summary>
    /// Role tokens used by <see cref="BaseComboBoxPainter.GetIconTint"/> to pick the
    ...
    internal abstract class BaseComboBoxPainter : IComboBoxPainter
    {
        ...
        public virtual void Paint(Graphics g, BeepComboBox owner, Rectangle drawingRect)
        {
            ...
            // Get layout rectangles directly from the passed drawingRect — same source
            // used by DrawContent when it called _helper.CalculateLayout(DrawingRect, ...).
            _helper.CalculateLayout(drawingRect, out Rectangle textAreaRect, out Rectangle buttonRect, out Rectangle imageRect);

            // Apply clear-button carveout to textAreaRect (mirrors DrawContent logic)
            if (owner.ShowClearButton && !owner.ClearButtonRect.IsEmpty)
            {
                textAreaRect = new Rectangle(textAreaRect.X, textAreaRect.Y,
                    Math.Max(1, textAreaRect.Width - owner.ClearButtonRect.Width), textAreaRect.Height);
            }
        ...
        /// (e.g., floating labels for <see cref="MaterialOutlinedComboBoxPainter"/>).
        /// </summary>
