...
{
    /// <summary>
    /// Arc Linux theme context menu painter with Arc GTK theme's flat design and subtle shadows
    /// Features Arc's signature flat aesthetic, minimalism, and clean typography
    /// </summary>
    public class ArcLinuxContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.ArcLinux;
        ...
        {
            // Arc Linux clean white background
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (item.IsEnabled)
            {
                if (visuallySelected)
                {
                    ...
                    {
                        // Arc blue selection
                        using (var brush = new SolidBrush(Color.FromArgb(255, 230, 240, 250)))
                        {
                            g.FillPath(brush, path);
                        }

                        // Subtle Arc border
                        using (var pen = new Pen(Color.FromArgb(255, 77, 128, 179), 1))
                        {
                            g.DrawPath(pen, path);
                        }
                    }
                ...
                else if (isHovered)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        using (var brush = new SolidBrush(Color.FromArgb(15, 77, 128, 179)))
                        {
                            g.FillPath(brush, path);
                        }
                    }
            ...
            // Arc Linux checkbox with flat theme
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawArcLinuxCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            var textColor = !item.IsEnabled ? Color.FromArgb(255, 150, 150, 150) :
                           ...
                           isHovered ? Color.FromArgb(255, 77, 128, 179) :
        ...
        private void DrawArcLinuxCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
