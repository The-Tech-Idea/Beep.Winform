...
{
    public class CartoonContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.Cartoon;
        ...
        public int GetPreferredItemHeight() => 38;
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            foreach (var item in items)
            {
                ...
                if (IsSeparator(item))
                {
                    DrawCartoonSeparator(g, owner, item, y, metrics);
                    y += metrics.SeparatorHeight + 4;
                    continue;
                }
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (metrics.ShowElevation)
            {
                ...
                shadowBounds.Offset(4, 4);
                using (var shadowPath = GetRoundedRect(shadowBounds, metrics.BorderRadius))
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawCartoonCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            {
                DrawCartoonArrow(g, itemRect, !item.IsEnabled, metrics, textColor);
            }
        }
        ...
        private void DrawCartoonCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
            ...
            if (isChecked)
            {
                using (var pen = new Pen(Color.FromArgb(255, 105, 180), 3))
                {
                    pen.StartCap = LineCap.Round;
                    pen.EndCap = LineCap.Round;
                    g.DrawLine(pen, rect.X + 4, rect.Y + rect.Height / 2, 
                        rect.X + rect.Width / 2, rect.Bottom - 5);
                    g.DrawLine(pen, rect.X + rect.Width / 2, rect.Bottom - 5, 
                        rect.Right - 4, rect.Y + 4);
                }
            }
        }

        private void DrawCartoonArrow(Graphics g, Rectangle itemRect, bool isDisabled, 
            ContextMenuMetrics metrics, Color arrowColor)
        {
            ...
            using (var pen = new Pen(arrowColor, 3))
            {
                ...
                g.DrawLine(pen, arrowRect.X + 6, midY - 6, arrowRect.Right - 6, midY);
                g.DrawLine(pen, arrowRect.Right - 6, midY, arrowRect.X + 6, midY + 6);
            }
        }

        private void DrawCartoonSeparator(Graphics g, BeepContextMenu owner, SimpleItem item, int y, ContextMenuMetrics metrics)
        {
            if (!owner.ShowSeparators) return;
            int lineY = y + metrics.SeparatorHeight / 2;
            using (var pen = new Pen(metrics.SeparatorColor, 2))
            {
