...
{
    public class ChatBubbleContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.ChatBubble;
        ...
        public int GetPreferredItemHeight() => 34;
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            {
                DrawChatBubbleTail(g, bounds, metrics);
            }
        }
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawChatBubbleCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            {
                DrawChatBubbleArrow(g, itemRect, !item.IsEnabled, metrics, textColor);
            }
        }
        ...
        private void DrawChatBubbleCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
        ...
        private void DrawChatBubbleArrow(Graphics g, Rectangle itemRect, bool isDisabled, 
            ContextMenuMetrics metrics, Color arrowColor)
        {
            ...
            using (var pen = new Pen(arrowColor, 2))
            {
                ...
                g.DrawLine(pen, arrowRect.X + 5, midY - 4, arrowRect.Right - 5, midY);
                g.DrawLine(pen, arrowRect.Right - 5, midY, arrowRect.X + 5, midY + 4);
            }
        }

        private void DrawChatBubbleTail(Graphics g, Rectangle bounds, ContextMenuMetrics metrics)
        {
            // Optional: Draw a small tail like a chat bubble (bottom-left corner)
            Point[] tailPoints = new Point[]
            {
                new Point(bounds.X + 20, bounds.Bottom),
                new Point(bounds.X + 10, bounds.Bottom + 8),
                new Point(bounds.X + 30, bounds.Bottom)
            };
        ...
        private void DrawSeparator(Graphics g, BeepContextMenu owner, SimpleItem item, int y, ContextMenuMetrics metrics)
        {
            ...
            using (var pen = new Pen(metrics.SeparatorColor, 1))
            {
                ...
                g.DrawLine(pen, metrics.ItemPaddingLeft + 4, lineY, 
                    owner.Width - metrics.ItemPaddingRight - 4, lineY);
            }
        }
