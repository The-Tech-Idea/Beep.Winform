...
{
    /// <summary>
    /// Dracula theme context menu painter with dark purple aesthetic
    /// Features vampire-inspired colors, subtle glows, and dark elegance
    /// </summary>
    public class DraculaContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.Dracula;
        ...
        {
            // Dracula dark background with subtle gradient
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                using (var brush = new LinearGradientBrush(bounds,
                    ...
                    Color.FromArgb(255, 45, 47, 64), // Slightly lighter purple-gray
                ...
                using (var glowPen = new Pen(Color.FromArgb(20, 189, 147, 249), 1))
                {
                    var innerRect = Rectangle.Inflate(bounds, -1, -1);
        ...
        {
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            using (var pen = new Pen(Color.FromArgb(80, 68, 71, 90), 1))
            {
                g.DrawPath(pen, path);
            }
        }
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            // Dracula selection with purple glow
            if (item.IsEnabled)
            {
                if (visuallySelected)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        ...
                        using (var brush = new SolidBrush(Color.FromArgb(255, 68, 71, 90)))
                        {
                            g.FillPath(brush, path);
                        }

                        // Purple glow border
                        using (var pen = new Pen(Color.FromArgb(189, 147, 249), 1))
                        {
                            g.DrawPath(pen, path);
                        }
                    }
                ...
                else if (isHovered)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        using (var brush = new SolidBrush(Color.FromArgb(30, 68, 71, 90)))
                        {
                            g.FillPath(brush, path);
                        }
                    }
            ...
            // Dracula checkbox with purple theme
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawDraculaCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
                x += metrics.CheckboxSize + metrics.IconTextSpacing;
            }

            // Icon with Dracula theming
            ...
            var textColor = !item.IsEnabled ? Color.FromArgb(255, 98, 114, 164) :
                           visuallySelected ? Color.FromArgb(255, 248, 248, 242) :
                           isHovered ? Color.FromArgb(255, 189, 147, 249) :
            ...
            if (owner.ShowShortcuts && !string.IsNullOrEmpty(shortcutDisplay))
            {
                var shortcutColor = !item.IsEnabled ? Color.FromArgb(255, 98, 114, 164) :
                                   visuallySelected ? Color.FromArgb(255, 255, 121, 198) :
                                   isHovered ? Color.FromArgb(255, 139, 233, 253) :
                                   Color.FromArgb(255, 98, 114, 164);
            ...
            if (item.Children != null && item.Children.Count > 0)
            {
                var arrowColor = !item.IsEnabled ? Color.FromArgb(255, 98, 114, 164) :
                               visuallySelected ? Color.FromArgb(255, 255, 121, 198) :
                               isHovered ? Color.FromArgb(255, 139, 233, 253) :
                               Color.FromArgb(255, 98, 114, 164);
        ...
        private void DrawDraculaCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
            using (var path = GetRoundedRect(rect, 2))
            {
                // Background
                var bgColor = isDisabled ? Color.FromArgb(255, 40, 42, 54) :
                           Color.FromArgb(255, 68, 71, 90);
                ...
                // Border
                var borderColor = isDisabled ? Color.FromArgb(255, 98, 114, 164) :
                              Color.FromArgb(255, 189, 147, 249);
                ...
                if (isChecked)
                {
                    var checkColor = isDisabled ? Color.FromArgb(255, 98, 114, 164) :
