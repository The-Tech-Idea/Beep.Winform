...
{
    public class GNOMEContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.GNOME;
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                ...
                var topGradient = new Rectangle(bounds.X, bounds.Y, bounds.Width, bounds.Height / 3);
                using (var gradBrush = new LinearGradientBrush(topGradient,
                    Color.FromArgb(10, Color.White),
                    Color.FromArgb(0, Color.White), 90f))
                {
                    g.FillRectangle(gradBrush, topGradient);
                }
            }
        }
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawGNOMECheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            {
                DrawGNOMEArrow(g, itemRect, !item.IsEnabled, metrics, textColor);
            }
        }
        ...
        private void DrawGNOMECheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
            ...
            using (var path = GetRoundedRect(rect, 3))
            {
                if (isChecked)
                {
                    using (var brush = new SolidBrush(Color.FromArgb(53, 132, 228)))
                    {
                        g.FillPath(brush, path);
                    }
                }
                
                using (var pen = new Pen(isChecked ? Color.FromArgb(53, 132, 228) : checkColor, 1.5f))
                {
                    g.DrawPath(pen, path);
                }
            }
        ...
        private void DrawGNOMEArrow(Graphics g, Rectangle itemRect, bool isDisabled, 
