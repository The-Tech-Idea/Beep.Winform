...
{
    /// <summary>
    /// Holographic theme context menu painter with futuristic holographic effects
    /// Features translucent layers, rainbow gradients, and sci-fi holographic aesthetics
    /// </summary>
    public class HolographicContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.Holographic;
        ...
        {
            // Holographic translucent background
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                ...
                using (var hologramBrush = new LinearGradientBrush(bounds,
                    Color.FromArgb(60, 255, 0, 255), // Magenta
                    Color.FromArgb(60, 0, 255, 255), // Cyan
                    LinearGradientMode.Vertical))
                {
                    g.FillPath(hologramBrush, path);
                }

                // Holographic interference pattern
                using (var interferenceBrush = new LinearGradientBrush(bounds,
                    Color.FromArgb(30, 255, 255, 0), // Yellow
                    Color.FromArgb(30, 0, 255, 0), // Green
                    LinearGradientMode.Horizontal))
                {
                    g.FillPath(interferenceBrush, path);
                }
            }
        }
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                ...
                using (var borderBrush = new LinearGradientBrush(bounds,
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            // Holographic translucent selection
            if (item.IsEnabled)
            {
                if (visuallySelected)
                {
                    ...
                    {
                        // Holographic selection with rainbow effect
                        using (var selectBrush = new LinearGradientBrush(itemRect,
                        ...
                        // Rainbow border
                        using (var borderBrush = new LinearGradientBrush(itemRect,
            ...
            // Holographic checkbox with rainbow theme
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawHolographicCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            var textColor = !item.IsEnabled ? Color.FromArgb(255, 150, 150, 150) :
                           ...
                           isHovered ? Color.FromArgb(255, 0, 100, 200) :
        ...
        private void DrawHolographicCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
