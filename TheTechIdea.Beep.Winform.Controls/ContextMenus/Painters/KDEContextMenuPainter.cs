...
{
    /// <summary>
    /// KDE theme context menu painter with KDE Plasma's blue accent and modern design
    /// Features KDE's signature blue (#3DAEE9), clean gradients, and Plasma aesthetics
    /// </summary>
    public class KDEContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.KDE;
        ...
        {
            // KDE clean background with subtle gradient
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                ...
                // Subtle KDE blue accent
                using (var accentBrush = new LinearGradientBrush(bounds,
                    Color.FromArgb(8, 61, 174, 233), // KDE blue
                    Color.FromArgb(0, 61, 174, 233),
                    LinearGradientMode.Vertical))
                {
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (item.IsEnabled)
            {
                if (visuallySelected)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        ...
                        // KDE blue border
                        using (var pen = new Pen(Color.FromArgb(255, 61, 174, 233), 1))
                        {
                            g.DrawPath(pen, path);
                        }
                    }
                ...
                else if (isHovered)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        using (var brush = new SolidBrush(Color.FromArgb(20, 61, 174, 233)))
                        {
                            g.FillPath(brush, path);
                        }
                    }
            ...
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawKDECheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            var textColor = !item.IsEnabled ? Color.FromArgb(255, 150, 150, 150) :
                           ...
                           isHovered ? Color.FromArgb(255, 61, 174, 233) :
        ...
        private void DrawKDECheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
