...
{
    /// <summary>
    /// Neon theme context menu painter with bright neon colors and glowing effects
    /// Features electric pinks, blues, and greens with high contrast and glow effects
    /// </summary>
    public class NeonContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.Neon;
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                ...
                using (var glowBrush = new LinearGradientBrush(bounds,
                    ...
                    Color.FromArgb(15, 0, 255, 255), // Electric blue
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                // Bright neon pink border
                using (var pen = new Pen(Color.FromArgb(255, 255, 0, 255), 2))
                {
                    g.DrawPath(pen, path);
                }
                ...
                using (var glowPen = new Pen(Color.FromArgb(150, 0, 255, 255), 1))
                {
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (item.IsEnabled)
            {
                if (visuallySelected)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        ...
                        using (var brush = new SolidBrush(Color.FromArgb(255, 20, 20, 40)))
                        {
                            g.FillPath(brush, path);
                        }
                ...
                else if (isHovered)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        using (var brush = new SolidBrush(Color.FromArgb(30, 255, 0, 255)))
                        {
                            g.FillPath(brush, path);
                        }
                    }
            ...
            // Neon checkbox with glowing theme
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawNeonCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
        ...
        private void DrawNeonCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
            using (var path = GetRoundedRect(rect, 2))
            {
                // Background
                var bgColor = isDisabled ? Color.FromArgb(255, 30, 30, 50) :
