...
{
    /// <summary>
    /// Nord theme context menu painter with frost gradients and Nordic minimalism
    /// Features icy blue-white color scheme and subtle frost effects
    /// </summary>
    public class NordContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.Nord;
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            {
                using (var brush = new LinearGradientBrush(bounds,
                    ...
                    Color.FromArgb(255, 46, 52, 70), // Slightly lighter frost blue
                ...
                using (var frostBrush = new LinearGradientBrush(bounds,
                    Color.FromArgb(8, 200, 220, 240),
                    Color.FromArgb(0, 200, 220, 240),
                    LinearGradientMode.Vertical))
                {
                    g.FillPath(frostBrush, path);
                }
            }
        }
        ...
        {
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            using (var pen = new Pen(Color.FromArgb(67, 76, 94), 1))
            {
                g.DrawPath(pen, path);
            }
        }
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (item.IsEnabled)
            {
                if (visuallySelected)
                {
                    ...
                    {
                        // Frost blue selection
                        using (var brush = new SolidBrush(Color.FromArgb(255, 67, 76, 94)))
                        {
                            g.FillPath(brush, path);
                        }

                        // Icy border
                        using (var pen = new Pen(Color.FromArgb(136, 192, 208), 1))
                        {
                            g.DrawPath(pen, path);
                        }
                    }
                ...
                else if (isHovered)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        using (var brush = new SolidBrush(Color.FromArgb(20, 67, 76, 94)))
                        {
                            g.FillPath(brush, path);
                        }
                    }
            ...
            // Nord checkbox with frost theme
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawNordCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            // Nord frost text colors
            var textRect = new Rectangle(x, itemRect.Y,
                itemRect.Width - x - rightReserved,
                itemRect.Height);

            var textColor = !item.IsEnabled ? Color.FromArgb(255, 129, 161, 193) :
                           visuallySelected ? Color.FromArgb(255, 216, 222, 233) :
                           isHovered ? Color.FromArgb(255, 136, 192, 208) :
                           Color.FromArgb(255, 216, 222, 233);
            ...
            if (owner.ShowShortcuts && !string.IsNullOrEmpty(shortcutDisplay))
            {
                var shortcutColor = !item.IsEnabled ? Color.FromArgb(255, 129, 161, 193) :
                                   visuallySelected ? Color.FromArgb(255, 163, 190, 140) :
                                   isHovered ? Color.FromArgb(255, 235, 203, 139) :
                                   Color.FromArgb(255, 129, 161, 193);
            ...
            if (item.Children != null && item.Children.Count > 0)
            {
                var arrowColor = !item.IsEnabled ? Color.FromArgb(255, 129, 161, 193) :
                               visuallySelected ? Color.FromArgb(255, 163, 190, 140) :
                               isHovered ? Color.FromArgb(255, 235, 203, 139) :
                               Color.FromArgb(255, 129, 161, 193);
        ...
        private void DrawNordCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
            using (var path = GetRoundedRect(rect, 2))
            {
                // Background
                var bgColor = isDisabled ? Color.FromArgb(255, 46, 52, 64) :
                           Color.FromArgb(255, 67, 76, 94);
                ...
                // Frost border
                var borderColor = isDisabled ? Color.FromArgb(255, 129, 161, 193) :
                              Color.FromArgb(255, 136, 192, 208);
                ...
                if (isChecked)
                {
                    var checkColor = isDisabled ? Color.FromArgb(255, 129, 161, 193) :
                                 Color.FromArgb(255, 163, 190, 140);
        ...
        private void DrawSeparator(Graphics g, BeepContextMenu owner, SimpleItem item, int y, ContextMenuMetrics metrics)
        {
            ...
            using (var pen = new Pen(Color.FromArgb(60, 67, 76, 94), 1))
            {
