...
{
    /// <summary>
    /// One Dark theme context menu painter with Atom's popular dark theme colors
    /// Features One Dark's signature blue-gray palette and syntax highlighting colors
    /// </summary>
    public class OneDarkContextMenuPainter : IContextMenuPainter
    {
        public FormStyle Style => FormStyle.OneDark;
            ...
            ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            {
                using (var brush = new SolidBrush(Color.FromArgb(255, 40, 44, 52)))
                {
                    g.FillPath(brush, path);
                }

                // Subtle One Dark accent
                using (var accentBrush = new LinearGradientBrush(bounds,
                    Color.FromArgb(10, 97, 175, 239), // One Dark blue
                    Color.FromArgb(0, 97, 175, 239),
        ...
        {
            using (var path = GetRoundedRect(bounds, metrics.BorderRadius))
            using (var pen = new Pen(Color.FromArgb(255, 63, 69, 76), 1))
            {
                g.DrawPath(pen, path);
            }
        }
            ...
            bool isSelected, bool isHovered, ContextMenuMetrics metrics, IBeepTheme theme)
        {
            ...
            if (item.IsEnabled)
            {
                if (visuallySelected)
                {
                    ...
                    {
                        // One Dark selection background
                        using (var brush = new SolidBrush(Color.FromArgb(255, 56, 60, 69)))
                        {
                            g.FillPath(brush, path);
                        }

                        // One Dark blue border
                        using (var pen = new Pen(Color.FromArgb(255, 97, 175, 239), 1))
                        {
                            g.DrawPath(pen, path);
                        }
                    }
                ...
                else if (isHovered)
                {
                    using (var path = GetRoundedRect(itemRect, metrics.ItemBorderRadius))
                    {
                        using (var brush = new SolidBrush(Color.FromArgb(20, 97, 175, 239)))
                        {
                            g.FillPath(brush, path);
                        }
                    }
            ...
            // One Dark checkbox with blue theme
            if (owner.ShowCheckBox && item.IsCheckable)
            {
                ...
                DrawOneDarkCheckbox(g, checkRect, item.IsChecked, !item.IsEnabled, metrics);
            ...
            var textColor = !item.IsEnabled ? Color.FromArgb(255, 117, 120, 123) :
                           visuallySelected ? Color.FromArgb(255, 171, 178, 191) :
                           isHovered ? Color.FromArgb(255, 97, 175, 239) :
            ...
            if (owner.ShowShortcuts && !string.IsNullOrEmpty(shortcutDisplay))
            {
                var shortcutColor = !item.IsEnabled ? Color.FromArgb(255, 117, 120, 123) :
                                   visuallySelected ? Color.FromArgb(255, 171, 178, 191) :
                                   isHovered ? Color.FromArgb(255, 97, 175, 239) :
                                   Color.FromArgb(255, 127, 132, 142);
            ...
            if (item.Children != null && item.Children.Count > 0)
            {
                var arrowColor = !item.IsEnabled ? Color.FromArgb(255, 117, 120, 123) :
                               visuallySelected ? Color.FromArgb(255, 171, 178, 191) :
                               isHovered ? Color.FromArgb(255, 97, 175, 239) :
                               Color.FromArgb(255, 127, 132, 142);
        ...
        private void DrawOneDarkCheckbox(Graphics g, Rectangle rect, bool isChecked, bool isDisabled, ContextMenuMetrics metrics)
        {
            using (var path = GetRoundedRect(rect, 2))
            {
                // Background
                var bgColor = isDisabled ? Color.FromArgb(255, 56, 60, 69) :
                ...
                // One Dark blue border
                var borderColor = isDisabled ? Color.FromArgb(255, 63, 69, 76) :
                ...
                if (isChecked)
                {
                    var checkColor = isDisabled ? Color.FromArgb(255, 117, 120, 123) :
                                 Color.FromArgb(255, 97, 175, 239);
        ...
        private void DrawSeparator(Graphics g, BeepContextMenu owner, SimpleItem item, int y, ContextMenuMetrics metrics)
        {
            ...
            using (var pen = new Pen(Color.FromArgb(100, 63, 69, 76), 1))
            {
