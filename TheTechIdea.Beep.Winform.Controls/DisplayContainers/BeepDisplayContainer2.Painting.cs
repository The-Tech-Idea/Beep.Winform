    ...
    public partial class BeepDisplayContainer2
    {
            ...
            using (var path = CreatePartialRoundedPath(area, r, tl, tr, bl, br))
            {
                ...
                if (_useTabStripGradient && area.Width > 1 && area.Height > 1)
                {
                    Color endColor = _tabStripGradientEndColor != Color.Empty
                        ...
                        : ControlPaint.Dark(tabStripColor, 0.08f);
            ...
            bool isDisabled = false)
        {
            ...
            else if (isPressed)
            {
                ...
                bgTop      = ControlPaint.Dark(_tabBackColor, 0.18f);
                bgBottom   = ControlPaint.Dark(_tabBackColor, 0.10f);
                borderCol  = ControlPaint.Dark(_borderColor, 0.15f);
                iconAlpha  = 1f;
            }
            else if (isHovered)
            {
                // Hovered: lighter background, accent border.
                bgTop      = ControlPaint.Light(_tabBackColor, 0.25f);
                bgBottom   = ControlPaint.Light(_tabBackColor, 0.12f);
            ...
            else
            {
                ...
                bgTop      = ControlPaint.Light(_tabBackColor, 0.10f);
                bgBottom   = ControlPaint.Dark(_tabBackColor, 0.05f);
