...
namespace TheTechIdea.Beep.Winform.Controls
{
    public class BeepiForm : Form, IBeepModernFormHost
    {
        ...
        protected override void OnHandleCreated(EventArgs e){ base.OnHandleCreated(e); ApplyAcrylicEffectIfNeeded(); ApplyMicaBackdropIfNeeded(); }
        ...
        public virtual void ApplyTheme(){ SuspendLayout(); try { try { if (beepuiManager1.Theme != Theme) beepuiManager1.Theme = Theme; } catch { } Color newBackColor = _currentTheme?.BackColor ?? SystemColors.Control; if (newBackColor == Color.Transparent || newBackColor == Color.Empty) { newBackColor = SystemColors.Control; } BackColor = newBackColor; BorderColor = _currentTheme?.BorderColor ?? SystemColors.ControlDark; ApplyFormStyle(); try { _ribbon?.ApplyThemeFromBeep(_currentTheme); } catch { } } finally { ResumeLayout(true); Invalidate(); Update(); } }
        protected void ApplyFormStyle(){ // inline former partial method
            ApplyMetrics(_formStyle);
            switch (_formStyle)
            {
                case BeepFormStyle.Modern: ApplyThemeMapping(); _glowColor = Color.FromArgb(100, 72, 170, 255); break;
                case BeepFormStyle.Metro: ApplyThemeMapping(); break;
                case BeepFormStyle.Glass: ApplyThemeMapping(); _glowColor = Color.FromArgb(120, 255, 255, 255); break;
                case BeepFormStyle.Office: ApplyThemeMapping(); break;
                case BeepFormStyle.ModernDark: _borderRadius=8; _borderThickness=1; _shadowDepth=8; _enableGlow=true; _glowColor=Color.FromArgb(80,0,0,0); _glowSpread=10f; BackColor=Color.FromArgb(32,32,32); BorderColor=Color.FromArgb(64,64,64); break;
                case BeepFormStyle.Material: ApplyThemeMapping(); _borderThickness=0; _shadowDepth=5; _enableGlow=true; _glowColor=Color.FromArgb(60,0,0,0); _glowSpread=15f; BorderColor=Color.Transparent; break;
                case BeepFormStyle.Minimal: ApplyThemeMapping(); _shadowDepth=0; _enableGlow=false; break;
                case BeepFormStyle.Classic: BackColor=SystemColors.Control; BorderColor=SystemColors.ActiveBorder; _shadowDepth=0; _enableGlow=false; break;
                case BeepFormStyle.Custom: ApplyThemeMapping(); break;
            }
            SyncStyleToHelpers();
            Invalidate();
            ApplyAcrylicEffectIfNeeded();
            if (IsHandleCreated && _animateStyleChange) { _ = AnimateOpacityAsync(0.0,1.0,140); }
        }
        #endregion
        ...
        protected void RegisterMouseMoveHandler(Action<MouseEventArgs> handler){ if(handler!=null) _mouseMoveHandlers.Add(handler);} protected void RegisterMouseLeaveHandler(Action handler){ if(handler!=null) _mouseLeaveHandlers.Add(handler);} protected void RegisterMouseDownHandler(Action<MouseEventArgs> handler){ if(handler!=null) _mouseDownHandlers.Add(handler);} protected override void OnMouseMove(MouseEventArgs e){ if(_inpopupmode) return; base.OnMouseMove(e); foreach(var h in _mouseMoveHandlers) h(e);} protected override void OnMouseLeave(EventArgs e){ base.OnMouseLeave(e); Cursor=Cursors.Default; foreach(var h in _mouseLeaveHandlers) h(); } protected override void OnMouseDown(MouseEventArgs e){ base.OnMouseDown(e); foreach(var h in _mouseDownHandlers) h(e);} 
        #endregion
