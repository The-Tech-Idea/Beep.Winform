...
namespace TheTechIdea.Beep.Winform.Controls.Forms.Caption.Renderers
{
    ...
    internal sealed class ElementaryCaptionRenderer : ICaptionRenderer
    {
        ...
        public void Paint(Graphics g, Rectangle captionBounds, float scale, IBeepTheme theme, FormWindowState windowState, out Rectangle invalidatedArea)
        {
            ...
            int top = (int)(5 * scale);
            _closeRect = new Rectangle(captionBounds.Right - btn - 12, top, btn, btn);
            _maxRect   = new Rectangle(_closeRect.Left - btn - 8, top, btn, btn);
            _minRect   = new Rectangle(_maxRect.Left   - btn - 8, top, btn, btn);

            var fore = theme?.AppBarButtonForeColor ?? _host.ForeColor;
            using var p = new Pen(fore, 1.2f);

            // Minimize
            int y = _minRect.Y + (int)(_minRect.Height * 0.62f);
