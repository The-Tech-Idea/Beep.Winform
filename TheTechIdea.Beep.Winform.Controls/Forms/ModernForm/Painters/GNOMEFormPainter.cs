...
namespace TheTechIdea.Beep.Winform.Controls.Forms.ModernForm.Painters
{
    ...
    internal sealed class GNOMEFormPainter : IFormPainter, IFormPainterMetricsProvider, IFormNonClientPainter
    {
        ...
        private void PaintAdwaitaPillButtons(Graphics g, BeepiFormPro owner, Rectangle captionRect, FormPainterMetrics metrics)
        {
            ...
            int buttonHeight = 22;
            int buttonWidth = metrics.VisualButtonWidth;
            ...
            PaintPillButton(g, closeRect, Color.FromArgb(246, 97, 81), padding, buttonWidth, buttonHeight, FormHitAreaNames.Close, closeHovered);

            // Maximize button: Green pill
            PaintPillButton(g, maxRect, Color.FromArgb(51, 209, 122), padding, buttonWidth, buttonHeight, FormHitAreaNames.Maximize, maxHovered);

            // Minimize button: Blue pill
            PaintPillButton(g, minRect, Color.FromArgb(53, 132, 228), padding, buttonWidth, buttonHeight, FormHitAreaNames.Minimize, minHovered);

            // Theme/Style buttons if shown
            if (owner.ShowStyleButton)
            {
                var styleRect = owner.CurrentLayout.StyleButtonRect;
                PaintPillButton(g, styleRect, Color.FromArgb(145, 65, 172), padding, buttonWidth, buttonHeight, FormHitAreaNames.Style, styleHovered);
            }

            if (owner.ShowThemeButton)
            {
                var themeRect = owner.CurrentLayout.ThemeButtonRect;
                PaintPillButton(g, themeRect, Color.FromArgb(230, 97, 0), padding, buttonWidth, buttonHeight, FormHitAreaNames.Theme, themeHovered);
            }
        }
