...
namespace TheTechIdea.Beep.Winform.Controls.Forms.ModernForm.Painters
{
    ...
    internal sealed class KDEFormPainter : IFormPainter, IFormPainterMetricsProvider, IFormNonClientPainter
    {
        ...
        {
            return FormPainterMetrics.DefaultForCached(FormStyle.KDE, owner.UseThemeColors ? owner.CurrentTheme : null);
        }
        ...
        public void PaintBackground(Graphics g, BeepiFormPro owner)
        {
            ...
            FormPainterRenderHelper.PaintGradientBackground(g, owner.ClientRectangle,
                ShiftLuminance(metrics.BackgroundColor, 0.05f),
        ...
        public void PaintCaption(Graphics g, BeepiFormPro owner, Rectangle captionRect)
        {
            ...
            FormPainterRenderHelper.PaintGradientBackground(g, captionRect,
                ShiftLuminance(metrics.CaptionColor, 0.04f),
        ...
        private void PaintKDEPlasmaButtons(Graphics g, BeepiFormPro owner, Rectangle captionRect, FormPainterMetrics metrics)
        {
            ...
            PaintPlasmaButton(g, closeRect, Color.FromArgb(237, 21, 21), padding, buttonSize, FormHitAreaNames.Close, metrics, closeHovered);
            
            // Maximize button: Green with plasma wave
            PaintPlasmaButton(g, maxRect, Color.FromArgb(24, 218, 24), padding, buttonSize, FormHitAreaNames.Maximize, metrics, maxHovered);
            
            // Minimize button: Blue with plasma wave
            PaintPlasmaButton(g, minRect, Color.FromArgb(61, 174, 233), padding, buttonSize, FormHitAreaNames.Minimize, metrics, minHovered);
            
            // Theme/Style buttons if shown
            if (owner.ShowStyleButton)
            {
                var styleRect = owner.CurrentLayout.StyleButtonRect;
                PaintPlasmaButton(g, styleRect, Color.FromArgb(147, 115, 203), padding, buttonSize, FormHitAreaNames.Style, metrics, styleHovered);
            }

            if (owner.ShowThemeButton)
            {
                var themeRect = owner.CurrentLayout.ThemeButtonRect;
                PaintPlasmaButton(g, themeRect, Color.FromArgb(246, 116, 0), padding, buttonSize, FormHitAreaNames.Theme, metrics, themeHovered);
            }
        }
        ...
        private void PaintPlasmaButton(Graphics g, Rectangle buttonRect, Color baseColor, int padding, int size, string buttonType, FormPainterMetrics metrics, bool isHovered = false)
        {
            ...
            using (var path = CreateRoundedSquarePath(rect, 4))
            {
                ...
                using (var gradientBrush = new LinearGradientBrush(rect,
                    ShiftLuminance(baseColor, 0.15f),
                    ShiftLuminance(baseColor, -0.05f),
                    LinearGradientMode.Vertical))
                {
                ...
                Color borderColor = isHovered ? Color.FromArgb(100, 255, 255, 255) : ShiftLuminance(baseColor, -0.2f);
