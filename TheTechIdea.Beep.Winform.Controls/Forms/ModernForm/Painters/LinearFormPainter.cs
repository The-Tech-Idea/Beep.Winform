...
namespace TheTechIdea.Beep.Winform.Controls.Forms.ModernForm.Painters
{
    ...
    internal sealed class LinearFormPainter : IFormPainter, IFormPainterMetricsProvider, IFormNonClientPainter
    {
        ...
        private void PaintLinearButtons(Graphics g, BeepiFormPro owner, Rectangle captionRect, FormPainterMetrics metrics)
        {
            ...
            PaintLinearButton(g, closeRect, Color.FromArgb(239, 68, 68), padding, buttonSize, FormHitAreaNames.Close, owner, closeHovered);

            // Maximize button - minimal green
            PaintLinearButton(g, maxRect, Color.FromArgb(34, 197, 94), padding, buttonSize, FormHitAreaNames.Maximize, owner, maxHovered);

            // Minimize button - minimal blue
            PaintLinearButton(g, minRect, Color.FromArgb(59, 130, 246), padding, buttonSize, FormHitAreaNames.Minimize, owner, minHovered);

            // Theme/Style buttons if shown
            if (owner.ShowStyleButton)
            {
                var styleRect = owner.CurrentLayout.StyleButtonRect;
                PaintLinearButton(g, styleRect, Color.FromArgb(139, 92, 246), padding, buttonSize, FormHitAreaNames.Style, owner, styleHovered);
            }

            if (owner.ShowThemeButton)
            {
                var themeRect = owner.CurrentLayout.ThemeButtonRect;
                PaintLinearButton(g, themeRect, Color.FromArgb(245, 158, 11), padding, buttonSize, FormHitAreaNames.Theme, owner, themeHovered);
            }
        }
