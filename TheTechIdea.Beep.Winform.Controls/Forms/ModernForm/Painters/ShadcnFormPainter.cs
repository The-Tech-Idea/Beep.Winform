...
namespace TheTechIdea.Beep.Winform.Controls.Forms.ModernForm.Painters
{
    ...
    internal sealed class ShadcnFormPainter : IFormPainter, IFormPainterMetricsProvider, IFormNonClientPainter
    {
        ...
        {
            return FormPainterMetrics.DefaultForCached(FormStyle.Shadcn, owner);
        }
        ...
        private void PaintShadcnButtons(Graphics g, BeepiFormPro owner, Rectangle captionRect, FormPainterMetrics metrics)
        {
            ...
            PaintShadcnButton(g, closeRect, Color.FromArgb(239, 68, 68), padding, buttonSize, FormHitAreaNames.Close, closeHovered);

            // Maximize button - subtle green
            PaintShadcnButton(g, maxRect, Color.FromArgb(34, 197, 94), padding, buttonSize, FormHitAreaNames.Maximize, maxHovered);

            // Minimize button - subtle blue
            PaintShadcnButton(g, minRect, Color.FromArgb(59, 130, 246), padding, buttonSize, FormHitAreaNames.Minimize, minHovered);

            // Theme/Style buttons if shown
            if (owner.ShowStyleButton)
            {
                var styleRect = owner.CurrentLayout.StyleButtonRect;
                PaintShadcnButton(g, styleRect, Color.FromArgb(139, 92, 246), padding, buttonSize, FormHitAreaNames.Style, styleHovered);
            }

            if (owner.ShowThemeButton)
            {
                var themeRect = owner.CurrentLayout.ThemeButtonRect;
                PaintShadcnButton(g, themeRect, Color.FromArgb(245, 158, 11), padding, buttonSize, FormHitAreaNames.Theme, themeHovered);
            }
        }
