...
namespace  TheTechIdea.Beep.Winform.Controls.ImageManagement
{
    public static class ImageListHelper
    {
        ...
        public static List<ImageConfiguration> GetGraphicFilesLocationsFromEmbedded(string[] namespaces)
        {
            ...
            // Gather assemblies from multiple sources without DependencyContext
            ...
            // Also walk referenced assemblies to broaden coverage
            var queue = new Queue<Assembly>(assemblySet);
            while (queue.Count > 0)
            {
                ...
                foreach (var an in refs)
                {
                    try
                    {
                        ...
                        if (loaded != null && !loaded.IsDynamic && !assemblySet.Contains(loaded))
                        {
                            assemblySet.Add(loaded);
            ...
            var assemblies = assemblySet.ToList();
