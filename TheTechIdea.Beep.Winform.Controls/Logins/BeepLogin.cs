...
using TheTechIdea.Beep.Winform.Controls.Logins.Painters;
...
namespace TheTechIdea.Beep.Winform.Controls
{
    ...
    public class BeepLogin : BaseControl    
        ...
        private void ApplyViewType()
        {
            ...
            // Hide all controls by default; they will be made visible based on metrics
            HideAllControls();

            // Get painter for this view type
            var painter = LoginPainterFactory.CreatePainter(_viewType);
            if (painter == null) return;

            // Use _currentTheme if available (from ApplyTheme), otherwise use Theme property
            var activeTheme = _currentTheme ;
            var useTheme = UseThemeColors && activeTheme != null;

            // Get style config for this view type
            var styleConfig = LoginStyleHelpers.GetStyleConfigForView(_viewType, activeTheme, useTheme);
            
            // Calculate layout metrics using the painter
            Rectangle containerBounds = new Rectangle(
                loginPanel.Padding.Left,
                loginPanel.Padding.Top,
                loginPanel.Width - loginPanel.Padding.Left - loginPanel.Padding.Right,
                loginPanel.Height - loginPanel.Padding.Top - loginPanel.Padding.Bottom);

            var metrics = painter.CalculateMetrics(containerBounds, _viewType, styleConfig);

            // Apply metrics to controls (position and size)
            ApplyMetricsToControls(metrics);

            // Configure controls with appropriate styles, fonts, and themes
            ConfigureControlsWithHelpers(styleConfig);
        ...
        /// <summary>
        /// Applies layout metrics to controls (positions and sizes)
        /// </summary>
        private void ApplyMetricsToControls(LoginLayoutMetrics metrics)
        {
            // Title
            if (metrics.HasControl("Title"))
            {
                var bounds = metrics.GetControlBounds("Title");
                lblTitle.Visible = true;
                lblTitle.Location = new Point(bounds.X, bounds.Y);
                lblTitle.Size = new Size(bounds.Width, bounds.Height);
            }

            // Subtitle
            if (metrics.HasControl("Subtitle"))
            {
                var bounds = metrics.GetControlBounds("Subtitle");
                lblSubtitle.Visible = true;
                lblSubtitle.Location = new Point(bounds.X, bounds.Y);
                lblSubtitle.Size = new Size(bounds.Width, bounds.Height);
            }

            // Username
            if (metrics.HasControl("Username"))
            {
                var bounds = metrics.GetControlBounds("Username");
                txtUsername.Visible = true;
                txtUsername.Location = new Point(bounds.X, bounds.Y);
                txtUsername.Size = new Size(bounds.Width, bounds.Height);
            }

            // Password
            if (metrics.HasControl("Password"))
            {
                var bounds = metrics.GetControlBounds("Password");
            ...
            // Login Button
            if (metrics.HasControl("LoginButton"))
            {
                var bounds = metrics.GetControlBounds("LoginButton");
                btnLogin.Visible = true;
                btnLogin.Location = new Point(bounds.X, bounds.Y);
                btnLogin.Size = new Size(bounds.Width, bounds.Height);
            }

            // Forgot Password
            if (metrics.HasControl("ForgotPassword"))
            {
                var bounds = metrics.GetControlBounds("ForgotPassword");
                lnkForgotPassword.Visible = true;
                lnkForgotPassword.Location = new Point(bounds.X, bounds.Y);
                lnkForgotPassword.Size = new Size(bounds.Width, bounds.Height);
            }

            // Register Link
            if (metrics.HasControl("RegisterLink"))
            {
                var bounds = metrics.GetControlBounds("RegisterLink");
                lnkRegister.Visible = true;
                lnkRegister.Location = new Point(bounds.X, bounds.Y);
                lnkRegister.Size = new Size(bounds.Width, bounds.Height);
            }

            // Logo
            if (metrics.HasControl("Logo"))
            {
                var bounds = metrics.GetControlBounds("Logo");
                btnLogo.Visible = true;
                btnLogo.Location = new Point(bounds.X, bounds.Y);
                btnLogo.Size = new Size(bounds.Width, bounds.Height);
            }

            // Avatar
            if (metrics.HasControl("Avatar"))
            {
                var bounds = metrics.GetControlBounds("Avatar");
                btnAvatar.Visible = true;
                btnAvatar.Location = new Point(bounds.X, bounds.Y);
                btnAvatar.Size = new Size(bounds.Width, bounds.Height);
            }

            // Social Buttons
            if (metrics.HasControl("FacebookButton"))
            {
                var bounds = metrics.GetControlBounds("FacebookButton");
                btnFacebookLogin.Visible = true;
                btnFacebookLogin.Location = new Point(bounds.X, bounds.Y);
                btnFacebookLogin.Size = new Size(bounds.Width, bounds.Height);
            }

            if (metrics.HasControl("TwitterButton"))
            {
                var bounds = metrics.GetControlBounds("TwitterButton");
                btnTwitterLogin.Visible = true;
                btnTwitterLogin.Location = new Point(bounds.X, bounds.Y);
                btnTwitterLogin.Size = new Size(bounds.Width, bounds.Height);
            }

            if (metrics.HasControl("GoogleButton"))
            {
                var bounds = metrics.GetControlBounds("GoogleButton");
                btnGoogleLogin.Visible = true;
                btnGoogleLogin.Location = new Point(bounds.X, bounds.Y);
                btnGoogleLogin.Size = new Size(bounds.Width, bounds.Height);
            }

            // Remember Me
            if (metrics.HasControl("RememberMe"))
            {
                var bounds = metrics.GetControlBounds("RememberMe");
                chkRememberMe.Visible = true;
                chkRememberMe.Location = new Point(bounds.X, bounds.Y);
                chkRememberMe.Size = new Size(bounds.Width, bounds.Height);
            }
        }

        /// <summary>
        /// Configures controls with appropriate styles, fonts, and themes using helpers
        /// </summary>
        private void ConfigureControlsWithHelpers(LoginStyleConfig styleConfig)
        {
            // Ensure styleConfig has the correct ControlStyle
            if (styleConfig == null)
            {
                styleConfig = LoginStyleConfig.CreateForViewType(_viewType);
            }
            styleConfig.ControlStyle = ControlStyle;
            ...
            LoginControlHelpers.ConfigureLoginTextBox(
                txtUsername,
                _viewType,
                styleConfig,
                activeTheme,
            ...
            LoginControlHelpers.ConfigureLoginButton(
                btnLogin,
                _viewType,
                styleConfig,
                activeTheme,
            ...
            {
                lblTitle.Text = "Login";
                LoginControlHelpers.ConfigureLoginLabel(
                    lblTitle,
                    _viewType,
                    styleConfig,
                    activeTheme,
            ...
            if (lnkForgotPassword != null)
            {
                ...
                LoginControlHelpers.ConfigureLoginLink(
                    lnkForgotPassword,
                    _viewType,
                    styleConfig,
                    activeTheme,
            ...
            if (chkRememberMe != null)
            {
                ...
                {
                    chkRememberMe.ForeColor = activeTheme.PrimaryTextColor;
                }
            }

            // Configure circular buttons (logo and avatar)
            LoginControlHelpers.ConfigureCircularButton(
                btnLogo,
                _viewType,
                styleConfig,
                activeTheme,
                useTheme,
                LoginIconHelpers.GetRecommendedLogoPath());
            ...
            LoginControlHelpers.ConfigureCircularButton(
                ...
                activeTheme,
                useTheme,
                LoginIconHelpers.GetRecommendedAvatarPath());
        }
        ...
        {
            base.ApplyTheme();
            if (loginPanel == null || _isApplyingTheme) return;
            ...
            try
            {
                ...
                // Reapply view type to update controls with new theme colors
                // This will use _currentTheme in ConfigureControlsWithHelpers
                ApplyViewType();
            }
            finally
            {
        ...
        {
            base.OnResize(e);
            if (loginPanel != null && _viewType != LoginViewType.Simple) // Only reapply if not in simple mode
            {
                // Reapply view type to recalculate layout with new size
                ApplyViewType();
            }
        }
