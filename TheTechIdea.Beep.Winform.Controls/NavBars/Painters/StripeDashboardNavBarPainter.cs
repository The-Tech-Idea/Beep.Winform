...
namespace TheTechIdea.Beep.Winform.Controls.NavBars.Painters
{
    ...
    public sealed class StripeDashboardNavBarPainter : BaseNavBarPainter
    {
        ...
        {
            var itemRect = Rectangle.Inflate(selectedRect, -4, -4);
            
            // Subtle background
            using (var path = CreateRoundedPath(itemRect, 6))
            using (var bgBrush = new SolidBrush(Color.FromArgb(245, 245, 247)))
            {
        ...
        {
            var itemRect = Rectangle.Inflate(hoverRect, -4, -4);
            
            using (var path = CreateRoundedPath(itemRect, 6))
            using (var hoverBrush = new SolidBrush(Color.FromArgb(250, 250, 252)))
            {
