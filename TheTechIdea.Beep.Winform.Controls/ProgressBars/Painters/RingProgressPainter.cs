...
namespace TheTechIdea.Beep.Winform.Controls.ProgressBars.Painters
{
    internal sealed class RingProgressPainter : IProgressPainter, IProgressPainterV2
    {
        ...
        public void Paint(Graphics g, Rectangle bounds, IBeepTheme theme, BeepProgressBar owner, IReadOnlyDictionary<string, object> p)
        {
            ...
            if (isIndeterminate)
            {
                float sweep = 90f + 60f * (float)Math.Sin(state.IndeterminateOffset * Math.PI * 2);
                float startAngle = -90f + state.IndeterminateOffset * 360f;
                g.DrawArc(forePen, ringRect, startAngle, sweep);
            }
            else
            {
            ...
            {
                centerText = ProgressPainterParameterContracts.GetString(p, "CenterText", $"{(int)(owner.DisplayProgressPercentageAccessor * 100)}%");
            }
            ...
            g.DrawString(centerText, f, br, ringRect, sf);
        }
