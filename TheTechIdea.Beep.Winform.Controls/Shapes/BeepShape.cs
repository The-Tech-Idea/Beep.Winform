...
namespace TheTechIdea.Beep.Winform.Controls.Shapes
{
    ...
    public class BeepShape : BaseControl
    {
        ...
        public BaseControl ConnectedStart { get; set; }

        [Browsable(true)]
        [Category("Connection")]
        [Description("Control connected to the end of this shape")]
        public BaseControl ConnectedEnd { get; set; }
        ...
        private void DrawRectangle(Graphics g, Pen pen, Brush brush)
        {
            if (CornerRadius > 0)
            {
                using (var path = GraphicsExtensions.GetRoundedRectPath(DrawingRect, CornerRadius))
                {
        ...
        private void UpdateCachedPath()
        {
            ...
            switch (ShapeType)
            {
                case BeepShapeType.Rectangle:
                    if (CornerRadius > 0)
                        cachedPath = GraphicsExtensions.GetRoundedRectPath(DrawingRect, CornerRadius);
                    else
        ...
        public void ConnectTo(BaseControl target, bool asStart = true)
        {
            if (asStart)
