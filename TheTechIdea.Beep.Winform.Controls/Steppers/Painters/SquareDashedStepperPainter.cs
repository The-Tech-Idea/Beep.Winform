...
namespace TheTechIdea.Beep.Winform.Controls.Steppers.Painters
{
    public sealed class SquareDashedStepperPainter : IStepperPainter
    {
        ...
        public void PaintStep(Graphics g, StepPainterContext context, int stepIndex, Rectangle stepRect)
        {
            ...
            Color text = StepperThemeHelpers.GetStepTextColor(context.Theme ?? _theme, step.State);

            if (selected)
            {
                fill = StepperThemeHelpers.GetStepActiveColor(context.Theme ?? _theme);
                text = StepperThemeHelpers.GetStepTextColor(context.Theme ?? _theme, StepState.Active);
            }
            ...
            if (hovered && !selected)
            {
                using var hover = new SolidBrush(Color.FromArgb(14, (context.Theme ?? _theme).StepperItemHoverBackColor));
