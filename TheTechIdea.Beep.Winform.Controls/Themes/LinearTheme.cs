...
namespace TheTechIdea.Beep.Winform.Controls.Themes
{
    /// <summary>
    /// Linear theme:
    /// - Ultra-minimal, sleek design
    /// - Light gray background (#F5F5F5) or dark (#1A1A1A)
    /// - Minimal chrome, very subtle borders
    /// - Sans-serif typography (Inter/Segoe UI)
    /// - Minimal borders & clean look (pairs with LinearFormPainter)
    /// </summary>
    public class LinearTheme : IBeepTheme
    {
        // Identity
        public string ThemeName { get; } = "LinearTheme";
        ...
        public LinearTheme()
        {
            // --- Base palette (Linear - Light mode)
            var background = C(245, 245, 245);      // #F5F5F5
            var surface = C(255, 255, 255);         // #FFFFFF
            var foreground = C(26, 26, 26);        // #1A1A1A
            var border = C(230, 230, 230);          // #E6E6E6 - very subtle
            var primary = C(94, 106, 210);         // #5E6AD2 - Linear blue
            var secondary = C(139, 139, 139);     // #8B8B8B
            var accent = primary;
            var muted = C(250, 250, 250);          // #FAFAFA
            var mutedForeground = C(139, 139, 139); // #8B8B8B
            
            // Linear theme color aliases (replacing terminal green colors with Linear colors)
            ...
            ErrorColor = C(239, 68, 68);            // #EF4444 - red
            WarningColor = C(245, 158, 11);         // #F59E0B - amber
            SuccessColor = C(34, 197, 94);          // #22C55E - green
            ...
            DisabledBackColor = C(250, 250, 250);
            DisabledForeColor = C(200, 200, 200);
            DisabledBorderColor = C(240, 240, 240);
            BorderColor = border;
            ActiveBorderColor = accent;
            InactiveBorderColor = border;
            // --- AppBar / Caption (matches LinearFormPainter)
            AppBarBackColor = surface;
            AppBarForeColor = foreground;
            AppBarButtonBackColor = C(250, 250, 250);  // Ultra-minimal
            AppBarButtonForeColor = C(120, 120, 120);  // Subtle
            ...
            // Linear control colors (minimal)
            AppBarCloseButtonColor = ErrorColor;  // red
            AppBarMaxButtonColor = SuccessColor;  // green
            AppBarMinButtonColor = primary;       // Linear blue

            // Typography for AppBar (sans-serif, minimal)
            ...
            PaddingMedium = 10;
            PaddingLarge = 16;
            BorderRadius = 4;   // minimal rounded corners for Linear
            BorderSize = 1;      // Very thin borders
            ...
            // --- Buttons
            // Linear style: neutral default button, distinct from background/surface
            ...
            // Validate and auto-fix contrast issues (Linear theme standard contrast)
