...
{
    /// <summary>
    /// Next.js/Vercel theme:
    /// - Clean white background
    /// - Modern gradient accents (blue to purple)
    /// - Vercel-inspired aesthetic
    /// - Sans-serif typography (Inter/Segoe UI)
    /// - Rounded corners & modern look (pairs with NextJSFormPainter)
    /// </summary>
    public class NextJSTheme : IBeepTheme
    {
        // Identity
        public string ThemeName { get; } = "NextJSTheme";
        ...
        public NextJSTheme()
        {
            // --- Base palette (Next.js/Vercel)
            var background = C(255, 255, 255);      // #FFFFFF - white
            var surface = C(250, 250, 250);         // #FAFAFA - light gray
            var foreground = C(0, 0, 0);           // #000000 - pure black
            var border = C(229, 229, 229);          // #E5E5E5
            var primary = C(0, 112, 243);          // #0070F3 - Vercel blue
            var secondary = C(139, 92, 246);        // #8B5CF6 - purple accent
            var accent = primary;                   // Use primary as accent
            var muted = C(249, 250, 251);          // #F9FAFB
            var mutedForeground = C(107, 114, 128); // #6B7280
            
            // Theme color aliases (replacing terminal green colors with Next.js colors)
            ...
            ErrorColor = C(242, 19, 97);            // #F21361 - vibrant red
            WarningColor = C(245, 166, 35);         // #F5A623 - orange
            SuccessColor = C(0, 217, 255);         // #00D9FF - cyan
            ...
            DisabledForeColor = C(163, 163, 163);
            DisabledBorderColor = C(229, 229, 229);
            BorderColor = border;
            ActiveBorderColor = accent;
            InactiveBorderColor = border;
            // --- AppBar / Caption (matches NextJSFormPainter)
            AppBarBackColor = background;
            AppBarForeColor = foreground;
            AppBarButtonBackColor = muted;
            ...
            // Next.js control colors
            AppBarCloseButtonColor = ErrorColor;  // vibrant red
            AppBarMaxButtonColor = SuccessColor;  // cyan
            AppBarMinButtonColor = primary;       // Vercel blue

            // Typography for AppBar (sans-serif, modern)
            ...
            PaddingMedium = 10;
            PaddingLarge = 16;
            BorderRadius = 10;  // rounded corners for Next.js
            BorderSize = 1;
            ...
            // --- Buttons
            // Next.js style: neutral default button with subtle cool tint
            ButtonBackColor = C(234, 240, 250);
            ButtonForeColor = foreground;  // Theme text
            ButtonBorderColor = border;
            
            // Hover: darker neutral tint
            ButtonHoverBackColor = C(220, 230, 245);
            ButtonHoverForeColor = foreground;
            ButtonHoverBorderColor = primary;
            
            // Pressed: strongest neutral tint
            ButtonPressedBackColor = C(205, 220, 240);
            ButtonPressedForeColor = foreground;
            ButtonPressedBorderColor = primary;

            // Selected: gradient accent
            ButtonSelectedBackColor = secondary;  // Purple gradient
            ButtonSelectedForeColor = C(255, 255, 255);
            ButtonSelectedBorderColor = secondary;
            
            // Selected hover: darker gradient
            ButtonSelectedHoverBackColor = C(124, 58, 237);
            ButtonSelectedHoverForeColor = C(255, 255, 255);
            ButtonSelectedHoverBorderColor = secondary;

            // Error button: vibrant red
            ...
            // Validate and auto-fix contrast issues (Next.js theme standard contrast)
