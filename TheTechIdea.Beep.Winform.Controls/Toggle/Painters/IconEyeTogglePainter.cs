...
{
    /// <summary>
    /// Eye icon toggle - shows eye (ON/visible) and eye-off (OFF/hidden) icons
    /// </summary>
    internal class IconEyeTogglePainter : BeepTogglePainterBase
    {
        public IconEyeTogglePainter(BeepToggle owner, BeepToggleLayoutHelper layout)
            : base(owner, layout)
        {
        }
        ...
        protected override void PaintTrack(Graphics g, Rectangle trackRect, ControlState state)
        {
            ...
            using (var path = GetRoundedRectPath(trackRect, radius))
            {
                using (var brush = new LinearGradientBrush(
                    trackRect,
                    trackColor,
                    ControlPaint.Dark(trackColor, 0.1f),
        ...
        protected override void PaintThumb(Graphics g, Rectangle thumbRect, ControlState state)
        {
            ...
            if (state != ControlState.Disabled)
            {
                ...
                {
                    var shadowRect = new Rectangle(thumbRect.X + 1, thumbRect.Y + 2, thumbRect.Width, thumbRect.Height);
            ...
            if (state != ControlState.Disabled)
            {
                using (var highlightBrush = new LinearGradientBrush(
                    ...
                    LinearGradientMode.Vertical))
                {
                    var highlightRect = new Rectangle(thumbRect.X, thumbRect.Y, thumbRect.Width, thumbRect.Height / 2);
        ...
        protected override void PaintIcons(Graphics g, ControlState state)
        {
            ...
            // Get appropriate SVG icon path - Eye when ON (visible), EyeOff when OFF (hidden)
            string iconPath = Owner.IsOn ? SvgsUI.Eye : SvgsUI.EyeOff;
