...
{
    /// <summary>
    /// Like icon toggle - shows thumbs-up (ON/liked) and thumbs-down (OFF/disliked) icons
    /// </summary>
    internal class IconLikeTogglePainter : BeepTogglePainterBase
    {
        public IconLikeTogglePainter(BeepToggle owner, BeepToggleLayoutHelper layout)
            : base(owner, layout)
        {
        }
        ...
        protected override void PaintTrack(Graphics g, Rectangle trackRect, ControlState state)
        {
            ...
            using (var brush = new LinearGradientBrush(trackRect, trackColor, ShiftLuminance(trackColor, -0.1f), LinearGradientMode.Vertical))
            {
        ...
        protected override void PaintThumb(Graphics g, Rectangle thumbRect, ControlState state)
        {
            ...
            using (var brush = new LinearGradientBrush(thumbRect, thumbColor, ShiftLuminance(thumbColor, -0.05f), LinearGradientMode.Vertical))
        ...
        protected override void PaintIcons(Graphics g, ControlState state)
        {
            ...
            {
                iconColor = Color.FromArgb(255, 76, 175, 80); // Material Green for thumbs-up
            }
            else
            {
