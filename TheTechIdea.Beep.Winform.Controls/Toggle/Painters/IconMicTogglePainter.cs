...
{
    /// <summary>
    /// Microphone icon toggle - shows mic (ON/unmuted) and mic-off (OFF/muted) icons
    /// </summary>
    internal class IconMicTogglePainter : BeepTogglePainterBase
    {
        public IconMicTogglePainter(BeepToggle owner, BeepToggleLayoutHelper layout)
            : base(owner, layout)
        {
        }
        ...
        protected override void PaintIcons(Graphics g, ControlState state)
        {
            ...
            string iconPath = Owner.IsOn ? SvgsUI.Mic : SvgsUI.MicOff;
