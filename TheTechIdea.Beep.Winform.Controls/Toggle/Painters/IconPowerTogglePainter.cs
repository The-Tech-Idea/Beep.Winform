...
{
    /// <summary>
    /// Power icon toggle - shows power (ON) and zap-off (OFF/disabled) icons
    /// </summary>
    internal class IconPowerTogglePainter : BeepTogglePainterBase
    {
        public IconPowerTogglePainter(BeepToggle owner, BeepToggleLayoutHelper layout)
            : base(owner, layout)
        {
        }
        ...
        protected override void PaintIcons(Graphics g, ControlState state)
        {
            ...
            string iconPath = Owner.IsOn ? SvgsUI.Power : SvgsUI.ZapOff;
