...
namespace TheTechIdea.Beep.Winform.Controls.Widgets.Helpers.Painters.Media
{
    ...
    internal sealed class AvatarGroupPainter : WidgetPainterBase, IDisposable
    {
        ...
        public override void DrawContent(Graphics g, WidgetContext ctx)
        {
            ...
            DrawAvatarCluster(g, ctx.ContentRect, mediaItems.Where(x => x.IsAvatar).Take(6).ToList(), ctx.AccentColor);
        }

        private void DrawAvatarCluster(Graphics g, Rectangle rect, List<MediaItem> avatars, Color accentColor)
        {
            ...
            for (int i = 0; i < Math.Min(avatars.Count, 6); i++)
            {
                ...
                else
                {
                    ...
                    using var initialsFont = new Font(Owner.Font.FontFamily, avatarSize / 4, FontStyle.Bold);
            ...
            if (avatars.Count > 6)
            {
                ...
                using var extraTextBrush = new SolidBrush(Color.White);
                using var extraFont = new Font(Owner.Font.FontFamily, avatarSize / 5, FontStyle.Bold);
