...
{
    public partial class uc_PostgreSqlConnections : TemplateUserControl
    {
        PostgreSqlConnectionViewModel viewModel;

        public uc_PostgreSqlConnections()
        {
            InitializeComponent();
        }

        private void uc_PostgreSqlConnections_Load(object sender, EventArgs e)
        {
            Configure();
        }
        ...
        {
            viewModel = new PostgreSqlConnectionViewModel(Editor, VisManager);
            ...
            textBoxSslRootCertificate.DataBindings.Add("Text", viewModel, "SslRootCertificate", true, DataSourceUpdateMode.OnPropertyChanged);
            ...
            textBoxSearchPath.DataBindings.Add("Text", viewModel, "SearchPath", true, DataSourceUpdateMode.OnPropertyChanged);
            textBoxApplicationName.DataBindings.Add("Text", viewModel, "ApplicationName", true, DataSourceUpdateMode.OnPropertyChanged);
            checkBoxIncludeErrorDetail.DataBindings.Add("Checked", viewModel, "IncludeErrorDetail", true, DataSourceUpdateMode.OnPropertyChanged);

            // Setup driver type dropdown
            comboBoxDriverType.DataSource = viewModel.PostgreSqlDatabaseTypes;
            comboBoxDriverType.DisplayMember = "PackageName";
            comboBoxDriverType.ValueMember = "PackageName";
            comboBoxDriverType.DataBindings.Add("SelectedItem", viewModel, "SelectedPostgreSqlDatabaseType", true, DataSourceUpdateMode.OnPropertyChanged);
